...
<cfscript>
	...
	URI = URIClass.create("http://127.0.0.1:8091/pools");
	
	CouchBaseClient = createObject("java","com.couchbase.client.CouchbaseClient").init([URI], "test", "pa$$");
	
	CouchBaseClient.set("key", 0, "Hello Brad");
	
	writeDump(CouchBaseClient.get("key"));
	abort;
	*/
	cb = getModel(dsl="cacheBox:couchBase");
	cb.set("foo","test");
	writeDump(cb.get("foo"));
	abort;
